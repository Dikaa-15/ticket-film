...
@section('content')
<h1 class="text-xl mb-3 text-gray-900 font-bold">Film & Bioskop</h1>
...
<div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
            @foreach($filmBioskops as $fb)
            <tr class="hover:bg-gray-50 transition-all">
                <td class="px-4 py-3 font-medium text-gray-900">{{ $fb->film->title ?? '-' }}</td>
                <td class="px-4 py-3 font-medium text-gray-900">
                    @if($fb->bioskops && $fb->bioskops->count() > 0)
                    @foreach($fb->bioskops as $bioskop)
                    <span>{{ $bioskop->name }}</span><br>
                    @endforeach
                    @else
                    <span>-</span> {{-- Tampilkan teks fallback jika tidak ada bioskop --}}
                    @endif
                </td>
                <td class="px-4 py-3">
                    <span class="inline-flex divide-x divide-gray-300 overflow-hidden rounded border border-gray-300 bg-white shadow-sm">
                        <!-- Tombol Edit -->
                        <a href="{{ route('filmbioskop.edit', $fb->id) }}"
                        ...
                        <!-- Tombol Delete -->
                        <form action="{{ route('filmbioskop.destroy', $fb->id) }}" method="POST"
