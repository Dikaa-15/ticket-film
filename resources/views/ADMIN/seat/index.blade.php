...
@section('content')
<h1 class="text-xl mb-3 text-gray-900 font-bold">Seat</h1>
...
<div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-50 text-left font-medium text-gray-700">
            <tr>
                <th class="px-4 py-3">Nomor Seat</th>
                <th class="px-4 py-3">Studio</th>
                <th class="px-4 py-3">Bioskop</th>
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
            ...
            <tr class="hover:bg-gray-50 transition-all">
                <td class="px-4 py-3 font-medium text-gray-900">{{ $seat->seat_number }}</td>
                <td class="px-4 py-3">{{ $seat->studio->name }}</td>
                <td class="px-4 py-3">{{ $seat->bioskop->name }}</td>
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded text-white {{ $seat->is_available ? 'bg-green-500' : 'bg-red-500' }}">
                        {{ $seat->is_available ? 'Tersedia' : 'Tidak Tersedia' }}
                    </span>
                </td>
                <td class="px-4 py-3">
