...
<h1 class="text-xl mb-3 text-gray-900 font-bold">Bioskop</h1>
    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        ...
        <tbody class="divide-y divide-gray-100 bg-white">
            ...
            <tr class="hover:bg-gray-50 transition-all">
                <td class="px-4 py-3 font-medium text-gray-900">{{ $bioskop->name }}</td>
                <td class="px-4 py-3 text-gray-700">
                    {{ \Illuminate\Support\Str::limit($bioskop->address, 40) }}
                </td>
                <td class="px-4 py-3 text-gray-700">{{ $bioskop->contact }}</td>
                <td class="px-4 py-3">
                    <span class="inline-flex divide-x divide-gray-300 overflow-hidden rounded border border-gray-300 bg-white shadow-sm">
                        <!-- Tombol Edit -->
                        <a href="{{ route('bioskop.edit', $bioskop->id) }}"
                        ...
                        <!-- Tombol Delete -->
                        <form action="{{ route('bioskop.destroy', $bioskop->id) }}" method="POST"
